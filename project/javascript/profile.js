...
function init() {
  ...
  document.getElementById('post-img-url').addEventListener('input', previewImage);
...
function previewImage() {
  const url = document.getElementById('post-img-url').value.trim();
...
function submitPost() {
  const imageUrl = document.getElementById('post-img-url').value.trim();
  const caption  = document.getElementById('post-caption').value.trim();
  if (!caption && !imageUrl) { toast('Add an image or a caption'); return; }
  const posts = ls.get('posts') || [];
  posts.unshift({
    id: uid(), authorId: currentUser.id, imageUrl, caption,
    timestamp: Date.now(), likes: [], comments: [],
  });
  ls.set('posts', posts);
  closeAddModal(); renderPosts(); toast('Post shared!');
}
...
function deletePost() {
  ...
  closeModal(); renderPosts(); toast('Post deleted');
}
